  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `I just booked ${bookingDetails.farmName} for my getaway! Booking ID: ${bookingDetails.id}`,
        url: window.location.href
      });
    } else {
      // Fallback for browsers that don't support Web Share API
      navigator.clipboard.writeText(`I just booked ${bookingDetails.farmName} for my getaway! Booking ID: ${bookingDetails.id}`);
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3 text-sm">
                    <div className="flex items-start gap-2">
                      ...
                      <p>A confirmation email has been sent to {bookingDetails.guestEmail}</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0"></div>
                      <p>The property owner will contact you 24 hours before check-in</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0"></div>
                      <p>Please carry a valid ID proof for verification at check-in</p>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 bg-primary rounded-full mt-2 flex-shrink-0"></div>
