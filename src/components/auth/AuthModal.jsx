...
export default function AuthModal() {
    ...
    const initializeRecaptcha = async () => {
      try {
        ...
        if (!container) {
          ...
          container = document.createElement("div");
          container.id = "recaptcha-container";
          container.style.display = "block"; // Visible for debugging
          document.body.appendChild(container);
        } else {
        ...
        console.log("Firebase app initialized:", app !== null && auth !== null);

        // Initialize reCAPTCHA with visible mode for debugging
        console.log("Initializing reCAPTCHA...");
        recaptchaRef.current = new RecaptchaVerifier(
          auth,
          "recaptcha-container",
          {
            sitekey: "6Le0k3wrAAAAAEHdR1TqQgEyu5k082THA9pRUHmC",
            ...
            "expired-callback": () => {
              console.warn("reCAPTCHA expired");
              setIsRecaptchaReady(false);
              setRecaptchaError("Security verification expired. Please try again.");
            },
            "error-callback": (error) => {
              console.error("reCAPTCHA error:", error);
  ...
  const handleSendOtp = async (phoneNumber) => {
    try {
      ...
      const confirmation = await signInWithPhoneNumber(auth, `+91${raw}`, recaptchaRef.current);
      console.log("OTP sent successfully");
      setIsOtpSent(true);
    } catch (err) {
      console.error("Error sending OTP:", err);
      setRecaptchaError("Failed to send OTP. Please try again.");
    } finally {
      setLocalLoading(false);
    }
  };
  ...
  const handleOtpSubmit = async (otp) => {
    try {
      ...
      console.log("OTP confirmed, user logged in:", result);
    } catch (err) {
  ...
  return (
    <div>
      <div id="recaptcha-container" />
      {recaptchaError && <Alert variant="destructive">{recaptchaError}</Alert>}
        ...
        {isOtpSent ? (
          <>
            ...
            <Input
              ...
              placeholder="Enter OTP"
              maxLength={6}
              onChange={(e) => handleOtpSubmit(e.target.value)}
            />
            <Button disabled={localLoading}>Verify OTP</Button>
          </>
        ) : (
          <Button
            onClick={() => handleSendOtp(formattedPhone)}
            disabled={localLoading || !isRecaptchaReady || recaptchaError}
          >
